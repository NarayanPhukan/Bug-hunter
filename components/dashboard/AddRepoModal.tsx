...
export default function AddRepoModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-bg/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-bg2 border border-border rounded-2xl w-full max-w-lg shadow-2xl animate-scale-in">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <div>
            <h2 className="font-display text-base font-bold text-text">Add repository</h2>
            <p className="text-text3 text-xs mt-0.5">Select a GitHub repo to monitor</p>
          </div>
          <button onClick={onClose} className="text-text3 hover:text-text text-xl leading-none transition-colors">×</button>
        </div>
          ...
          {filtered.map(r => (
            <div key={r.id} className="flex items-center justify-between px-6 py-3 hover:bg-bg3 transition-colors">
              ...
              <button
                ...
                className="ml-4 flex-shrink-0 text-xs bg-indigo hover:bg-indigo3 text-white px-4 py-1.5 rounded-lg font-semibold transition-all disabled:opacity-50"
              >
        ...
        {error && (
          <div className="px-6 py-3 border-t border-border text-xs text-red">⚠ {error}</div>
        )}

        <div className="px-6 py-3 border-t border-border text-[11px] text-text3">
